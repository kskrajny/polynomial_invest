...
def experiment(data_delta_days, clf_names, instrument_list, delta_list, tau, future_length,
               polynomial_degree, file_names, start_date, final_date, skip_time_days, explain, candle_num,
               class_balance,
               feature_eng, feature_eng_name, labels, results_dir, classifiers, tresholds, spread_q):

    dfs = [pd.DataFrame() for _ in instrument_list]
    exp_df = pd.DataFrame()
        ...
        for clf_name in clf_names:
            ...
            try:
                stats, exp_res = single_training(instrument_list, date_from_train, date_from_test, date_to_test,
                                                 delta_list,
                                                 clf_name, tau, future_length, polynomial_degree, classifiers,
                                                 tresholds,
                                                 labels=labels, global_date_from=start_date, global_date_to=final_date,
                                                 explain=explain, candle_num=candle_num,
    ...
    save_results(dfs, exp_df, file_names, meta_text, future_length, results_dir, clf_names)
